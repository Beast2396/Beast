...
(function () {
  ...
  /* ── 06 writing ────────────────────────────────────────────────────── */
  /* no posts yet: drop the section and its nav entry rather than leaving a
     hole — and return before writing into an element that no longer exists */
  if (!S.writing.length) {
  ...
  var writingHost = $('#writing-list');
  if (writingHost) writingHost.innerHTML = (S.writing || []).map(function (w, i) {
    return entry({
